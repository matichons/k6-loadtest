...
export default async function () {
  ...
  try {
    ...
    // Submit the form
    await page.locator('form#add_course_user').evaluate((form) => form.submit());
    ...
    const successText = await page.locator('.alert-success');

    const formSubmitSuccess = check(successText, {
      'Form submitted successfully': (el) =>
        el && el.textContent().includes('สำเร็จ'),
    });

    formSubmitSuccess ? httpReqSuccess.add(1) : httpReqFailed.add(1);
